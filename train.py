...
from plot_functions import save_train, save_test_quanti, save_test_quali, save_dimwise_traversals, save_imgwise_traversals
...
class Run(object):
    ...
    def plot(self, data, WEIGHTS_PATH):
        ...
        if opts['dataset']=='celebA':
            idx_to_plot = [1,2,3,4,9]
        elif opts['dataset']=='3Dchairs':
            idx_to_plot = [0,1,6,7,8]
        else:
            idx_to_plot = np.range(opts['evaluate_num_pics'])
        data_to_plot = self.data.data_plot[idx_to_plot]
        latents = self.sess.run(self.encoded, feed_dict={
                            self.inputs_img: data_to_plot,
                            self.is_training: False})
        enc_var = np.ones(opts['zdim'])
        # create latent linespace
        ...
        # - ploting and saving
        if opts['dataset'] in ['3Dchairs', 'celebA']:
            save_test_quali(opts, self.data.data_plot[:num_pics],
                                        reconstructions,
                                        obs_transversal,
                                        generations,
                                        opts['exp_dir'])
        ...
        save_imgwise_traversals(opts, obs_transversal, opts['exp_dir'])
