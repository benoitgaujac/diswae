...
def linespace(opts,n,anchors,std):
    """
    Genereate linear grid space
        - n:  Num of steps in the interpolation
        - anchors: anchors points [nanchors,zdim]
        -std: standard deviation
    Return:
    list of linear interpolations. shape [nanchors,zdim,n,zdim]
    """
    interpolations = []
    ...
    for i in range(nanchors):
        ...
        interpolation = []
        inter = np.stack([anchors[i] for j in range(n)],axis=0)
        for d in range(opts['zdim']):
            int = np.stack([anchors[i] for j in range(n)],axis=0)
            int[:,d]=grid[d]
            interpolation.append(int)
        interpolations.append(np.array(interpolation))
